...
def main():
    ...
    generate_parser.add_argument('--rated_power', type=float, default=25, help='Rated power output')
    ...
    if args.command == 'generate_data':
        try:
            wind_speeds, power_outputs, wind_directions, temperatures = turbine_model.data_processor.generate_turbine_data(args.size, args.cut_in, args.rated_speed, args.rated_power, args.cut_out, args.shape, args.scale, wind_direction_range=args.wind_direction_range, temperature_range=args.temperature_range)
        ...
        try:
            ...
            turbine_model.train(data, power_outputs)
            utils.save_model(turbine_model, args.model_path)
